...
//    j$('#testml').on('click', function(event)
//    {
...
function Conference(phoneNr) // popup
{
    ...
    if (common.getuseengine() === global.ENGINE_WEBRTC && common.IsMizuServer() === false && common.GetParameter('conf_engineswitcheoffered') !== 'true')
    {
...
/* no need for this, because it reloads the page
    ...
    j$('#adialog_positive').on('click', function (event)
    {
        ...
        common.PutToDebugLog(5,'EVENT, call Conference ok onclick: ' + textboxval);
    ...
    if (featureset > 5)
    {
        if (availableFunc.indexOf('conference') >= 0 &&
            (
                ...
                || (common.getuseengine() === global.ENGINE_WEBRTC && (common.GetConfigBool('usingmizuserver', false) === true || common.IsMizuWebRTCEmbeddedServer() === true || common.IsMizuWebRTCGateway() === true))
            ))
...
//        if (availableFunc.indexOf('conference') >= 0 || (common.IsMizuServer() === true && IsConferenceRoom() === true)) // for conference rooms
    ...
    {
        j$(menuId).append( '<li id="' + MENUITEM_CALL_NUMPAD + '"><a data-rel="back">' + stringres.get('menu_numpad') + '</a></li>' ).listview('refresh');
    }
